...
window.validateGivenSum = function (fonction) {
  ...
  // envoyer_preparation_payer_fractionner // addition_liste
  //  id="commandes-table-contenu" class="BF-ligne-deb" data-total-addition-en-cours="0" data-idtable="4" data-nomtable="S04" data-total-commandes="37.99" data-reste-a-payer="37.99"
  ...
  try {
    ...
    if (isNaN(sum)) {
      ...
      if ((glob.actionAValider === 'envoyer_preparation_payer' || glob.actionAValider === 'vente_directe' || glob.actionAValider === 'addition_liste') && sum < totalAchat) {
...
export function popupConfirme(moyenPaiement, nom, fonction){
  ...
  let frag = `<div id="popup-cashless-confirm" class="BF-col popup-cashless-confirm-conteneur">
    <h1>
      <div class="BF-ligne" data-i8n="confirmPayment, capitalize" role="status" aria-label="${getTranslate('confirmPayment')}">
        Confirmez le paiement
      </div>
      <div class="BF-ligne">
        <span data-i8n="by" style="margin-right:6px;"></span>
        <span role="status" aria-label="${getTranslate('paymentMethod')}">${nom}</span>
      </div>
    </h1>
  ...
  if (moyenPaiement === 'espece') {
    ...
    if (glob.appConfig.periph !== 'FPI') {
      ...
      frag += `
        ...
        <input id="given-sum" class="addition-fractionnee-input keyboard-use" keyboard-type="numpad" onclick="keyboard.run(this)">
        <small id="given-sum-msg-erreur"></small>
      `
    }
  }
